...
/** `scanlyfix-example.com-2026-08-27.csv` — sortable, and obvious in a downloads folder. */
export function filename(scan: ReportScan, extension: string): string {
  const safeHost = host(scan.url).replace(/[^a-z0-9.-]/gi, '-')
  return `scanlyfix-${safeHost}-${scan.createdAt.toISOString().slice(0, 10)}.${extension}`
}
...
export function buildMarkdown(input: ReportInput): string {
  ...
  out.push(`# ScanlyFix report — ${host(scan.url)}`)
  out.push('')
  ...
  if (scan.checkErrors.length > 0) {
    ...
    out.push('These are ScanlyFix-side failures, not issues with the site.')
    out.push('')
  ...
  out.push('---')
  out.push('')
  out.push(`Generated by ScanlyFix · scan \`${scan.id}\``)
  ...
  const errorList =
    scan.checkErrors.length > 0
      ? `<h2>Checks that could not run (${scan.checkErrors.length})</h2>` +
        '<p class="note">These are ScanlyFix-side failures, not issues with the site.</p><ul>' +
        scan.checkErrors.map((e) => `<li><code>${esc(e.checkId)}</code> — ${esc(e.message)}</li>`).join('') +
        '</ul>'
      : ''

  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><title>ScanlyFix report — ${esc(host(scan.url))}</title>
<style>
...
<body>
  ...
  ${errorList}
  <p class="note" style="margin-top:16pt">Generated by ScanlyFix · scan ${esc(scan.id)}</p>
</body></html>`
}
