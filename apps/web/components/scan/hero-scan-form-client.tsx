/**
 * A client island that mounts the Supabase Auth provider around the hero's
 * scan form so it can read signed-in state.
 *
 ...
 *
 * SSR caveat: the Supabase auth context only exists on the client, so the
 * form is gated behind a "mounted" state and rendered as a static placeholder
 * on the server. The placeholder is the same visual — same input, same
 * button — so the landing page does not flash an empty box on hydration. The
 * auth gate is the only behaviour that needs the client; everything else
 * (validation, the button label) is identical in both states.
 */
'use client'
